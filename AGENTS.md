...
> **Note for agents**: Prioritize retrieval-led reasoning over pretrained-knowledge-led reasoning. Use available skills (e.g. `create-drumlogue-unit`) rather than generating instructions from memory.
...
Additional instruction files provide detailed guidance for specific areas and are referenced in `opencode.json`:

- `instructions/cpp.instructions.md` - DSP code style, real-time constraints, ARM optimization
- `instructions/build-system.instructions.md` - Makefiles, config.mk, container builds
- `instructions/testing.instructions.md` - Desktop test harnesses, hardware validation
- `instructions/debugging.instructions.md` - Build, runtime, and DSP debugging

## Specialized Agents

- `@drumlogue-dsp-expert` - Expert mode for DSP development (see `.opencode/agents/drumlogue-dsp-expert.md`)
